...
func test_zone_border_top_edge() -> bool:
	...
	assert_eq(_get_cell_zone(pos, []), "border", "y=0 is border")
	for y: int in range(ZONE_BORDER_DIST):
		assert_eq(_get_cell_zone(Vector2i(30, y), []), "border",
...
func test_zone_border_left_edge() -> bool:
	...
	for x: int in range(ZONE_BORDER_DIST):
		assert_eq(_get_cell_zone(Vector2i(x, 20), []), "border",
...
func test_zone_center() -> bool:
	...
	assert_eq(_get_cell_zone(pos, []), "center", "(30,20) is center")
	pos = Vector2i(10, 10)
	assert_eq(_get_cell_zone(pos, []), "center", "(10,10) is center")
	return true
...
func test_zone_near_rival() -> bool:
	...
	assert_eq(_get_cell_zone(Vector2i(20, 20), rival_cells), "near_rival", "rival cell itself")
	# Within distance
	assert_eq(_get_cell_zone(Vector2i(22, 22), rival_cells), "near_rival", "2 tiles away")
	assert_eq(_get_cell_zone(Vector2i(25, 25), rival_cells), "near_rival", "exactly 5 tiles away (boundary)")
	# Outside distance
	assert_eq(_get_cell_zone(Vector2i(26, 20), rival_cells), "center", "6 tiles away = center")
	assert_eq(_get_cell_zone(Vector2i(20, 26), rival_cells), "center", "6 tiles away = center")
	return true
...
func test_zone_border_beats_near_rival() -> bool:
	...
	assert_eq(_get_cell_zone(pos, rival_cells), "border",
	...
	assert_eq(_get_cell_zone(interior_near, rival_cells), "near_rival",
...
func test_growth_cost_border() -> bool:
	...
	assert_eq(_get_growth_cost(Vector2i(0, 20), []), 3.0, "border cost = 3.0")
	assert_eq(_get_growth_cost(Vector2i(GRID_W - 1, 20), []), 3.0, "right border cost = 3.0")
	return true

func test_growth_cost_center() -> bool:
	"""Center zone cost = 5.0 GP."""
	assert_eq(_get_growth_cost(Vector2i(30, 20), []), 5.0, "center cost = 5.0")
	return true

func test_growth_cost_near_rival() -> bool:
	"""Near-rival zone cost = 7.0 GP."""
	var rival_cells: Array[Vector2i] = [Vector2i(30, 20)]
	assert_eq(_get_growth_cost(Vector2i(30, 20), rival_cells), 7.0, "near-rival cost = 7.0")
	return true

func test_growth_cost_ordering() -> bool:
	"""Growth costs follow the difficulty order: border < center < near_rival."""
	assert_true(ZONE_COST_BORDER < ZONE_COST_CENTER, "border < center")
	assert_true(ZONE_COST_CENTER < ZONE_COST_NEAR_RIVAL, "center < near_rival")
	return true
...
func test_zone_tint_colors() -> bool:
	...
	assert_true(border_tint.r < center_tint.r, "border has less red than center")
	assert_true(rival_tint.g < center_tint.g, "rival has less green than center")
	assert_true(border_tint.a != center_tint.a, "zones have distinct alpha")
	return true
...
func test_zone_tint_all_different() -> bool:
	...
	assert_ne(border_tint, center_tint, "border ≠ center")
	assert_ne(center_tint, rival_tint, "center ≠ rival")
	assert_ne(border_tint, rival_tint, "border ≠ rival")
	return true
